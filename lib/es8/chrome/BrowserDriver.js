...
class BrowserDriver {
	...
	constructor(nick) {
		...
		this._debugging_port = Math.floor(Math.random() * (10000 - 9000) + 9000)
		//this._debugging_port = 9221
	}
	...
	_initialize(callback) {
		...
		const childOptions = [
			"--remote-debugging-port=" + this._debugging_port,
		...
		if (this._options.headless) {
			...
			childOptions.push("--no-gpu")
			childOptions.push("--disable-software-rasterizer")
		...
		const checkDebuggerPort = () => {
			setTimeout(() => {
				const socket = net.createConnection(this._debugging_port)
	...
	_newTabDriver(uniqueTabId, callback) {
		const connectToTab = (cdpTarget) => {
			...
			CDP({ port: this._debugging_port, target: cdpTarget }, (client) => {
				const tab = new TabDriver(this._debugging_port, uniqueTabId, this._options, client, cdpTarget.id)
				tab._init((err) => {
					if (err) {
						callback(err)
					} else {
		...
		CDP.List({ port: this._debugging_port }, (err, tabs) => {
			if (err) {
				...
				if ((uniqueTabId === 1) && (tabs.length === 1) && (tabs[0].url === "about:blank")) {
					...
					CDP.New({ port: this._debugging_port }, (err, tab) => {
						if (err) {
